...
Instead of fetching raw data and masking in memory, it generates SQL that masks sensitive columns at the source:

```sql
-- Original
SELECT username, password FROM users LIMIT 5;

-- Generated by Profiling Service
SELECT `username`, '***MASKED***' AS `password` FROM users LIMIT 5;
```

This ensures that sensitive data (PII) never leaves the database server during the profiling process, providing a strong security guarantee.
